...
-- ════════════════════════════════════════════════════════════
--  UNLOCK ALL — SHARED MODULES / HOOKS
--  Hooks are installed before the UI and can be toggled at runtime.
-- ════════════════════════════════════════════════════════════
local Players           = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local lp                = Players.LocalPlayer
local LocalPlayer       = lp
local controllers       = lp:WaitForChild("PlayerScripts"):WaitForChild("Controllers")
...
pcall(function()
    ...
    local cosmeticModule = modules and modules:FindFirstChild("CosmeticLibrary")
    if cosmeticModule then
        CosmeticLibrary = instanceSafeRequire(cosmeticModule, 5)
    end
end)

pcall(function()
    local dataModule = controllers:FindFirstChild("PlayerDataController")
    if dataModule then
        DataController = instanceSafeRequire(dataModule, 5)
    end
end)

pcall(function()
    local emoteModule = controllers:FindFirstChild("EmoteController")
    if emoteModule then
        EmoteController = instanceSafeRequire(emoteModule, 5)
    end
end)

local instanceUnlockAllEnabled = false

local function instanceIsEmote(c)
    if not c then return false end
    local t = c.Type or ""
    return t == "Dance" or t == "Emote"
end

if CosmeticLibrary and CosmeticLibrary.OwnsCosmetic then
    local originalOwnsCosmetic = CosmeticLibrary.OwnsCosmetic
    CosmeticLibrary.OwnsCosmetic = function(self, inv, name, weapon)
        if instanceUnlockAllEnabled then
            if name and name:find("MISSING_") then
                return originalOwnsCosmetic(self, inv, name, weapon)
            end
            if name and CosmeticLibrary.Cosmetics and CosmeticLibrary.Cosmetics[name] then
                return true
            end
        end
        return originalOwnsCosmetic(self, inv, name, weapon)
...
if DataController and DataController.Get then
    local originalDataControllerGet = DataController.Get
    DataController.Get = function(self, key)
        local data = originalDataControllerGet(self, key)

        if instanceUnlockAllEnabled and key == "CosmeticInventory" then
            local proxy = {}
            if data then
...
if EmoteController and EmoteController.GetEmotes then
    ...
    EmoteController.GetEmotes = function(self)
        local emotes = originalGetEmotes(self) or {}

        if instanceUnlockAllEnabled and CosmeticLibrary and CosmeticLibrary.Cosmetics then
            for name, c in pairs(CosmeticLibrary.Cosmetics) do
                if c and instanceIsEmote(c) and not emotes[name] then
                    emotes[name] = {
                        Name     = name,
                        Type     = c.Type,
                        ObjectID = c.ObjectID,
...
-- Nameless.wtf / Linoria UI bootstrap
-- UI-only migration: the feature/runtime code below stays unchanged.
...
local function loadRemoteModule(url, label)
    ...
    if not chunk then
        error(string.format("[Instance] %s failed to compile: %s", label, tostring(err)), 2)
    end
    local ok, result = pcall(chunk)
    if not ok then
        error(string.format("[Instance] %s failed to initialize: %s", label, tostring(result)), 2)
    end
    return result
end

local Library = loadRemoteModule(NamelessLibraryURL, "Nameless Library")
local ThemeManager = loadRemoteModule(NamelessThemeManagerURL, "Nameless ThemeManager")
local SaveManager = loadRemoteModule(NamelessSaveManagerURL, "Nameless SaveManager")
...
-- Compatibility for the Instance script's notification configuration calls.
if type(Library.ConfigureNotifications) ~= "function" then
    function Library:ConfigureNotifications(_)
        return self
    end
end
...
local unlockAllBox = Tabs.Misc:AddLeftGroupbox('unlock all')
unlockAllBox:AddToggle('unlock_all', {
    Text = 'unlock all',
    Default = false,
    Callback = function(Value)
        instanceUnlockAllEnabled = Value
        if Value and DataController and DataController.CurrentData then
            task.defer(function()
                pcall(function()
...
-- ════════════════════════════════════════════════════════════
--  INSTANCE AC BYPASS — MESSAGE 5
-- ════════════════════════════════════════════════════════════
local LogService        = game:GetService("LogService")
local ScriptContext     = game:GetService("ScriptContext")
local CollectionService = game:GetService("CollectionService")
local trackedInstanceConnections = {}

local function track(conn)
    if conn then
        table.insert(trackedInstanceConnections, conn)
    end
    return conn
end
    ...
    for _, v in pairs(getgc(true)) do
        if typeof(v) == "function" then
            ...
            if ok and type(src) == "string" and string.find(src, "AnalyticsPipelineController") then
                hookfunction(v, newcclosure(function(...) return wait(9e9) end))
            end
        end
    end
end)
...
pcall(function()
    local remote = ReplicatedStorage:WaitForChild("Remotes", 5)
        and ReplicatedStorage.Remotes:WaitForChild("AnalyticsPipeline", 5)
        and ReplicatedStorage.Remotes.AnalyticsPipeline:WaitForChild("RemoteEvent", 5)
...
pcall(function()
    for _, name in ipairs({"Kick", "kick"}) do
        ...
        if type(fn) == "function" then
            local old
            old = hookfunction(fn, newcclosure(function(self, ...)
...
pcall(function()
    if typeof(getrenv) ~= "function" then return end
    local renv = getrenv()
    if not renv or not renv.setmetatable then return end
    local origSet
    origSet = hookfunction(renv.setmetatable, newcclosure(function(tbl, mt)
        pcall(function()
            if mt and type(mt) == "table" and rawget(mt, "__mode") then
                local wm = rawget(mt, "__mode")
                if wm == "kv" or wm == "v" or wm == "k" then
                    local st = debug.traceback()
                    if st:find("MiscellaneousController")
                        or st:find("LocalScript3")
                        or st:find("CameraSecurity")
                        or st:find("AnalyticsPipelineController") then
                        tbl, mt = {1, 2, 3}, {}
                    end
                end
            end
        end)
        return origSet(tbl, mt)
    end))
end)
...
pcall(function()
    local function patchTable(tbl)
        ...
        if rawget(tbl, "IsWithinOOBPart") or rawget(tbl, "OOB_DEFAULT_WARN_DELAY") then
            tbl.IsWithinOOBPart = function() return nil end
            tbl.GetOOBWarnDelay = function() return math.huge end
            tbl.GetOOBKillDelay = function() return math.huge end
            tbl.OOB_DEFAULT_WARN_DELAY = math.huge
            tbl.OOB_DEFAULT_KILL_DELAY = math.huge
        end
        if rawget(tbl, "IsWithinTaggedParts") and not rawget(tbl, "_pOOB") then
            local orig = tbl.IsWithinTaggedParts
            tbl.IsWithinTaggedParts = function(self, tag, ...)
                if tag == "OutOfBoundsPart" then return {} end
                if tag == "OutOfBoundsSafePart" then return true end
                return orig(self, tag, ...)
            end
            tbl._pOOB = true
        end
        if rawget(tbl, "IsOutOfBounds") and rawget(tbl, "_CheckKill") then
            tbl.IsOutOfBounds = function() return nil end
            tbl.Update = function() end
            tbl._CheckKill = function() return false end
        end
    end
    pcall(function() patchTable(require(ReplicatedStorage.Modules.Utility)) end)
    pcall(function() patchTable(require(ReplicatedStorage.Modules.GameplayUtility)) end)
    pcall(function()
        local m = ReplicatedStorage.Modules:FindFirstChild("OutOfBoundsMachine")
        if m then patchTable(require(m)) end
    end)
    pcall(function()
        for _, p in pairs(CollectionService:GetTagged("OutOfBoundsPart")) do
            CollectionService:RemoveTag(p, "OutOfBoundsPart")
        end
        for _, p in pairs(CollectionService:GetTagged("OutOfBoundsSafePart")) do
            CollectionService:RemoveTag(p, "OutOfBoundsSafePart")
        end
        track(CollectionService:GetInstanceAddedSignal("OutOfBoundsPart"):Connect(function(p)
            pcall(CollectionService.RemoveTag, CollectionService, p, "OutOfBoundsPart")
        end))
        track(CollectionService:GetInstanceAddedSignal("OutOfBoundsSafePart"):Connect(function(p)
            pcall(CollectionService.RemoveTag, CollectionService, p, "OutOfBoundsSafePart")
        end))
    end)
end)
...
-- ════════════════════════════════════════════════════════════
--  UNLOCK ALL COSMETICS — INSTANCE API
-- ════════════════════════════════════════════════════════════
local function instanceUnlockAllCosmetics()
    if not (CosmeticLibrary and CosmeticLibrary.Cosmetics) then
        return false, "CosmeticLibrary not loaded"
    end

    instanceUnlockAllEnabled = true

    local count = 0
    for cosName, cosData in pairs(CosmeticLibrary.Cosmetics) do
        if cosName and cosData then
            count = count + 1
        end
    end
...
local function instanceUnlockAllForWeapon(weaponName)
    if not (CosmeticLibrary and CosmeticLibrary.Cosmetics) then
        return false, "CosmeticLibrary not loaded"
    end
...
getgenv().InstanceUnlockAll = {
    cosmetics = instanceUnlockAllCosmetics,
    weapon = instanceUnlockAllForWeapon,
}
