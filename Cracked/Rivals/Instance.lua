...
-- Nameless.wtf UI migration
-- Replace the embedded UI implementation with the current upstream Nameless.wtf files.
...
Library = loadstring(NamelessLibrarySource)()

-- Use the matching Nameless.wtf addons rather than the embedded addon copies.
ThemeManager = loadstring(game:HttpGet(NamelessThemeManagerURL))()
SaveManager = loadstring(game:HttpGet(NamelessSaveManagerURL))()

-- Make the Nameless.wtf menu responsive on touch devices.
-- Do not apply a second UIScale to the Nameless library.
-- A second scale was making the effective menu size inconsistent with the
-- Library's own layout, especially on desktop.
local function UpdateNamelessScale()
    return
end
...
-- Ragebot has its own top-level tab so it cannot disappear inside a cramped
-- Combat tabbox.  The existing ragebot implementation/configuration remains
-- unchanged below; only its UI parent is made explicit and directly visible.
...
local function alignInstanceMenuTabs()
    ...
    -- Reserve a stable header region for the six main tabs.  The old
    -- alignment used the title's measured TextBounds as the starting point,
    -- which could change during load and leave too little width for tabs.
    -- The library's title already owns the first ~300 px of the header.
    local headerStart = 300
    local headerWidth = math.max(inner.AbsoluteSize.X - headerStart - 8, 1)

    tabArea.Position = UDim2.new(0, headerStart, 0, 4)
    tabArea.Size = UDim2.new(0, headerWidth, 0, 22)
    ...
    for _, button in ipairs(tabArea:GetChildren()) do
...
task.delay(0.1, alignInstanceMenuTabs)
task.delay(0.5, alignInstanceMenuTabs)

pcall(function()
    if Window and Window.Holder then
        Window.Holder:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
            task.defer(alignInstanceMenuTabs)
        end)
    end
end)
...
local rageui = {
    targetgroup = Tabs.Ragebot:AddRightTabbox()
}
...
--// Mobile UI support: responsive scaling + floating show/hide button
--// This layer is intentionally separate so the toggle remains usable while the menu is hidden.
do
    local Players = game:GetService("Players")
    local UserInputService = game:GetService("UserInputService")
    local CoreGui = game:GetService("CoreGui")
    ...
    -- Keep the complete main interface at native size.  The previous mobile
    -- UIScale was also active on desktop, which made the menu and its tab
    -- contents needlessly small and contributed to cramped tab layout.
    local mainScale = ScreenGui:FindFirstChild("MobileResponsiveScale")
    if not mainScale then
        mainScale = Instance.new("UIScale")
        mainScale.Name = "MobileResponsiveScale"
        mainScale.Parent = ScreenGui
    end
    mainScale.Scale = 1

    -- Separate toggle GUI, unaffected by the main UI's UIScale.
    local toggleGui = Instance.new("ScreenGui")
    toggleGui.Name = "NamelessMobileToggle"
    toggleGui.ResetOnSpawn = false
    toggleGui.IgnoreGuiInset = true
    toggleGui.DisplayOrder = 2147483647
    toggleGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    pcall(function() ProtectGui(toggleGui) end)
    ...
    toggleButton.Size = UDim2.fromOffset(46, 36)
    ...
    local function setMenuVisible(visible)
        if Window and Window.Holder then
            Window.Holder.Visible = visible
        else
            -- Fallback for interfaces that do not expose Window.Holder.
            for _, child in ipairs(ScreenGui:GetChildren()) do
                if child:IsA("GuiObject") and child ~= toggleButton then
                    child.Visible = visible
                end
            end
        end
        toggleButton.Text = visible and "UI" or "OPEN"
    end

    toggleButton.Activated:Connect(function()
        if Window and Window.Holder then
            setMenuVisible(not Window.Holder.Visible)
        end
    end)

    -- Keep the toggle styled if the theme changes.
    task.spawn(function()
    ...
    -- Allow the floating button to be dragged with mouse or touch.
