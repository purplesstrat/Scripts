getgenv().SleepyHookACDebug = false
SleepyHook=Instance
...
local _fakeEv
pcall(function()
    _fakeEv = SleepyHook.new("RemoteEvent")
...
pcall(function()
    ...
    for _i = 1, #_gc do
        ...
        if not _ok2 or not _scr or typeof(_scr) ~= "SleepyHook" then continue end
...
do
local skinState = {equipped={}, favorites={}, hooked=false, originals={}, mods={}, viewingProfile=nil, saveFile="SleepyHook_skins_v2.json"}
...
local function acLog(reason)
    if getgenv().SleepyHookACDebug then
...
print("LOADED ALPHA SleepyHook ACB ")

local function SleepyHookShowLoadingNotification()
    local RunService = game:GetService("RunService")
    local guiParent = (gethui and gethui()) or game:GetService("CoreGui")
    local gui = Instance.new("ScreenGui")
    gui.Name = "SleepyHookLoadingNotification"
...
getgenv().SleepyHookModuleCache = getgenv().SleepyHookModuleCache or {}

local function SleepyHookSafeRequire(moduleRef, timeoutSec)
    local cache = getgenv().SleepyHookModuleCache
    local key = typeof(moduleRef) == "SleepyHook" and moduleRef:GetFullName() or tostring(moduleRef)
    if cache[key] ~= nil then return cache[key] end
    local deadline = timeoutSec and (os.clock() + timeoutSec) or math.huge
    local lastErr
    while os.clock() < deadline do
        if typeof(moduleRef) == "SleepyHook" and not moduleRef.Parent then
            task.wait(0.1)
        else
    ...
    error("[SleepyHook] module load failed: " .. key .. " (" .. tostring(lastErr) .. ")")
end

getgenv().SleepyHookRequire = SleepyHookSafeRequire

local function SleepyHookPreloadCoreModules()
    local replicatedStorage = game:GetService("ReplicatedStorage")
    local players = game:GetService("Players")
    ...
    task.spawn(function() pcall(function() SleepyHookSafeRequire(modulesFolder:WaitForChild("EnumLibrary", 4)) end) end)
    task.spawn(function() pcall(function() SleepyHookSafeRequire(modulesFolder:WaitForChild("Utility", 4)) end) end)
    local controllers = playerScripts:WaitForChild("Controllers", 8)
    task.spawn(function() pcall(function() SleepyHookSafeRequire(controllers:WaitForChild("FighterController", 4)) end) end)
    task.spawn(function() pcall(function() SleepyHookSafeRequire(controllers:WaitForChild("CameraController", 4)) end) end)
end

local function SleepyHookHideErrorPromptsStep()
...
local function SleepyHookRunLoadingBootstrap()
    local runService = game:GetService("RunService")
    local dismissLoading = SleepyHookShowLoadingNotification()
    local hideErrors = true
    local hideConn = runService.RenderStepped:Connect(function()
        if hideErrors then SleepyHookHideErrorPromptsStep() end
    end)

    pcall(SleepyHookPreloadCoreModules)
...
local function SleepyHookPlainReplace(str, find, replace)
    local startPos = 1
    while true do
...
getgenv().SleepyHookSliderFillState = getgenv().SleepyHookSliderFillState or {}
getgenv().SleepyHookSliderFollowRate = getgenv().SleepyHookSliderFollowRate or 11

getgenv().SleepyHookSetSliderFill = function(fill, targetX, hideBorder, sliderObj, forceSnap)
    if not fill or not fill.Parent then return end
    targetX = math.floor(tonumber(targetX) or 0)
    local stateMap = getgenv().SleepyHookSliderFillState
    if forceSnap then
...
if not getgenv().SleepyHookSliderFillBound then
    getgenv().SleepyHookSliderFillBound = true
    RunService.RenderStepped:Connect(function(dt)
        local stateMap = getgenv().SleepyHookSliderFillState
        if not stateMap then return end
        dt = math.clamp(tonumber(dt) or 0, 1 / 1000, 1 / 15)
        local follow = getgenv().SleepyHookSliderFollowRate or 11
...
if not NamelessLibraryChunk then
    error("[SleepyHook] Nameless Library failed to compile: " .. tostring(NamelessLibraryError))
end
...
local SleepyHook_ACCENT = Color3.fromRGB(71, 119, 182)

local function applySleepyHookAccentTheme()
    ...
    Library.AccentColor = SleepyHook_ACCENT
    Library.OutlineColor = Color3.fromRGB(31, 31, 31)
    if Library.AccentColorDark == nil then
        local h, s, v = Color3.toHSV(SleepyHook_ACCENT)
...
applySleepyHookAccentTheme()
getgenv().SleepyHookApplyAccentTheme = applySleepyHookAccentTheme



local SleepyHook_MENU_DISPLAY_ORDER = 2147483646
local SleepyHook_COSMETIC_DISPLAY_ORDER = 2147483647
local SleepyHook_KEYBIND_LIST_ORDER = 80
local SleepyHook_GAMEPLAY_OVERLAY_ORDER = 10

local function applySleepyHookUiLayering()
    pcall(function()
        if Library and Library.ScreenGui then
            Library.ScreenGui.DisplayOrder = SleepyHook_MENU_DISPLAY_ORDER
            Library.ScreenGui.IgnoreGuiInset = true
            Library.ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        end
    end)
    pcall(function()
        local st = getgenv().SleepyHookCosmeticUIState
        if st and st.gui then
            st.gui.DisplayOrder = SleepyHook_COSMETIC_DISPLAY_ORDER
            st.gui.IgnoreGuiInset = true
            st.gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        end
    end)
    pcall(function()
        local kb = getgenv().SleepyHookKeybindList
        if kb and kb.gui then
            kb.gui.DisplayOrder = SleepyHook_KEYBIND_LIST_ORDER
            kb.gui.IgnoreGuiInset = true
            kb.gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        end
    end)
end
getgenv().SleepyHookApplyUiLayering = applySleepyHookUiLayering
...
pcall(function()
    if isfile("SleepyHook/silentload.txt") and readfile("SleepyHook/silentload.txt") == "true" then
...
applySleepyHookUiLayering()

getgenv().SleepyHookMenuCursor = getgenv().SleepyHookMenuCursor or {}

local function ensureSleepyHookMenuCursor()
    local mc = getgenv().SleepyHookMenuCursor
...
local function isSleepyHookMenuOpen()
...
local function shouldSuppressGameplayOverlays()
    return isSleepyHookMenuOpen()
end

ensureSleepyHookMenuCursor()

local MENU_CURSOR_HZ = 240
local MENU_CURSOR_BIND = "SleepyHookMenuCursorDraw"
local menuCursorAccum = 0

local function drawSleepyHookMenuCursor()
    local mc = getgenv().SleepyHookMenuCursor
    if not mc or not mc.cursor or not mc.outline then return end
    local open = isSleepyHookMenuOpen()
    if not open then
...
pcall(function()
    RunService:BindToRenderStep(MENU_CURSOR_BIND, Enum.RenderPriority.Last.Value + 10, function(dt)
        ...
        drawSleepyHookMenuCursor()
    end)
end)

getgenv().SleepyHookIsMenuOpen = isSleepyHookMenuOpen

getgenv().SleepyHookLanguage = getgenv().SleepyHookLanguage or "english"

local SleepyHookWeaponTranslations = {
    korean = {
...
getgenv().SleepyHookTranslateWeapon = function(name)
    if not name or type(name) ~= "string" then return name end
    local lang = getgenv().SleepyHookLanguage or "english"
    if lang == "english" then return name end
    local pack = SleepyHookWeaponTranslations[lang]
    return (pack and pack[name]) or name
end

getgenv().SleepyHookTranslateLabel = function(text)
    if not text or type(text) ~= "string" then return text end
    local lang = getgenv().SleepyHookLanguage or "english"
    if lang == "english" then return text:lower() end
    local translated = getgenv().SleepyHookTranslateWeapon(text)
    if translated ~= text then return translated:lower() end
    return text:lower()
end

getgenv().SleepyHookGetUiFont = function()
    if getgenv().SleepyHookLanguage == "korean" and getgenv().SleepyHookUIFont then
        return getgenv().SleepyHookUIFont
    end
    if Library and Library.Font then
        return Library.Font
    end
    return getgenv().SleepyHookUIFont or Font.fromEnum(Enum.Font.Roboto)
end

getgenv().SleepyHookL = function(key, ...)
    local lang = getgenv().SleepyHookLanguage or "english"
    local pack = SleepyHookLocaleTable[lang] or SleepyHookLocaleTable.english
    local template = pack[key] or SleepyHookLocaleTable.english[key] or key
    if select("#", ...) > 0 then
...
local SleepyHookFontHttp = game:GetService("HttpService")

local SleepyHookFontSources = {
    english = { file = "SleepyHook_ui_en.otf", fontFile = "SleepyHook_ui_en.font", name = "SleepyHookUI_En", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
    korean = { file = "SleepyHook_ui_ko.otf", fontFile = "SleepyHook_ui_ko.font", name = "SleepyHookUI_Ko", url = "https://github.com/googlefonts/noto-cjk/raw/main/Sans/OTF/Korean/NotoSansCJKkr-Regular.otf" },
    spanish = { file = "SleepyHook_ui_es.otf", fontFile = "SleepyHook_ui_es.font", name = "SleepyHookUI_Es", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
}

local function loadSleepyHookLanguageFont(lang)
    lang = lang or getgenv().SleepyHookLanguage or "english"
    local src = SleepyHookFontSources[lang] or SleepyHookFontSources.english
    local ok, face = pcall(function()
        if isfile and writefile and getcustomasset then
            ...
            writefile(src.fontFile, SleepyHookFontHttp:JSONEncode(fontdata))
            return Font.new(getcustomasset(src.fontFile))
        end
        return nil
    end)
    if ok and face then
        getgenv().SleepyHookUIFont = face
        return face
    end
    getgenv().SleepyHookUIFont = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium)
    return getgenv().SleepyHookUIFont
end

getgenv().SleepyHookReloadLanguageFont = function(lang)
    lang = lang or getgenv().SleepyHookLanguage or "english"
    if lang == "english" and Library and Library.Font then
        getgenv().SleepyHookUIFont = Library.Font
        return Library.Font
    end
    return loadSleepyHookLanguageFont(lang)
end
task.defer(function()
    pcall(function()
        getgenv().SleepyHookReloadLanguageFont(getgenv().SleepyHookLanguage)
    end)
end)

getgenv().SleepyHookDrawingTextPool = getgenv().SleepyHookDrawingTextPool or {}

getgenv().SleepyHookTrackDrawingText = function(obj)
    table.insert(getgenv().SleepyHookDrawingTextPool, obj)
    pcall(function()
        local face = getgenv().SleepyHookUIFont
        if face and obj.FontFace ~= nil then obj.FontFace = face end
    end)
    return obj
end

getgenv().SleepyHookApplyDrawingUIFont = function()
    local face = getgenv().SleepyHookUIFont
    if not face then return end
    for _, obj in ipairs(getgenv().SleepyHookDrawingTextPool) do
        pcall(function()
...
getgenv().SleepyHookApplyUiFont = function(obj, textSize)
    if not obj then return end
    if textSize then obj.TextSize = textSize end
    local font = getgenv().SleepyHookGetUiFont()
    pcall(function()
...
getgenv().SleepyHookApplyUIFont = function()
    local face = getgenv().SleepyHookGetUiFont()
    if not face then return end
    getgenv().SleepyHookUIFont = face
        ...
        for _, box in pairs(_G.ESPObjects) do
            if box and box.text then
                for _, lbl in pairs(box.text) do
                    if typeof(lbl) == "SleepyHook" and lbl:IsA("TextLabel") then
                        lbl.FontFace = face
                    end
                end
            end
    ...
    if getgenv().SleepyHookApplyDrawingUIFont then pcall(getgenv().SleepyHookApplyDrawingUIFont) end
    if getgenv().SleepyHookApplyKbListTheme then pcall(getgenv().SleepyHookApplyKbListTheme) end
end

getgenv().SleepyHookHudFonts = {
    ...
    { n = "custom", draw = 2, face = function() return getgenv().SleepyHookUIFont or Font.fromEnum(Enum.Font.Gotham) end },
}
getgenv().SleepyHookHudFontNames = {}
for i, slot in ipairs(getgenv().SleepyHookHudFonts) do
    getgenv().SleepyHookHudFontNames[i] = slot.n
end
getgenv().SleepyHookHudFontIdx = 1
getgenv().SleepyHookHudDrawFont = 2

getgenv().SleepyHookHudFontIndexByName = function(name)
    for i, slot in ipairs(getgenv().SleepyHookHudFonts) do
...
getgenv().SleepyHookApplyHudFont = function(idx)
    idx = math.clamp(tonumber(idx) or getgenv().SleepyHookHudFontIdx or 1, 1, #getgenv().SleepyHookHudFonts)
    local slot = getgenv().SleepyHookHudFonts[idx]
    getgenv().SleepyHookHudFontIdx = idx
    getgenv().SleepyHookHudDrawFont = slot.draw or 2
    local face
    pcall(function() face = slot.face() end)
    if face then getgenv().SleepyHookEspUiFont = face end
    if _G.ESPObjects then
        for _, box in pairs(_G.ESPObjects) do
            if box and box.text then
                for _, lbl in pairs(box.text) do
                    if typeof(lbl) == "SleepyHook" and lbl:IsA("TextLabel") and face then
                        lbl.FontFace = face
                    end
                end
            end
    ...
    for _, obj in ipairs(getgenv().SleepyHookDrawingTextPool or {}) do
        pcall(function()
            if obj.Font ~= nil then obj.Font = getgenv().SleepyHookHudDrawFont end
            if face and obj.FontFace ~= nil then obj.FontFace = face end
        end)
    end
end
...
-- SleepyHook Nameless UI safety layer.
...
do
    ...
    do
        local cfg = getgenv().SleepyHookLuaConfig or {}
        getgenv().SleepyHookLuaConfig = cfg
    ...
    do
        ...
        uiCore:AddToggle('MH_Settings_Show', { Text = 'show menu on load', Default = true, Callback = function(v) getgenv().SleepyHookShowOnLoad = v end })
        ...
        uiCore:AddLabel('SleepyHook / Nameless.wtf UI', true)
    end
end
...
local function alignSleepyHookMenuTabs()
    if not Window or not Window.Holder then return end
    local inner = Window.Holder:FindFirstChildOfClass("Frame")
    if not inner then return end
...
task.defer(alignSleepyHookMenuTabs)
...
local Utility           = SleepyHookSafeRequire(ReplicatedStorage.Modules.Utility)
local EnumLibrary       = SleepyHookSafeRequire(ReplicatedStorage.Modules.EnumLibrary)
local LocalPlayer       = Players.LocalPlayer
...
getgenv().SleepyHookGetAspectStretch = getAspectStretch
getgenv().SleepyHookWorldToScreen = worldToScreen
getgenv().SleepyHookScreenAnchor = screenAnchor
getgenv().SleepyHookScreenCenter = screenCenter
getgenv().SleepyHookGetUnstretchedCameraCFrame = getUnstretchedCameraCFrame

_G.Features = _G.Features or {}
_G.Features.CrossbowSoundId = "rbxassetid://165946246"
getgenv().SleepyHookCombatLastShotAt = getgenv().SleepyHookCombatLastShotAt or 0

local function markCombatShot()
    getgenv().SleepyHookCombatLastShotAt = tick()
end
getgenv().SleepyHookMarkCombatShot = markCombatShot

getgenv().SleepyHookWorldToScreenEsp = function(worldPos, cam)
...
local fovScreenGui = SleepyHook.new("ScreenGui")
fovScreenGui.Name            = "FOVScreenGui"
fovScreenGui.DisplayOrder    = SleepyHook_GAMEPLAY_OVERLAY_ORDER
...
local function buildfov(name, cfg)
    local container = SleepyHook.new("Frame")
    container.Name                   = name
    ...
    local fill = SleepyHook.new("Frame")
    ...
    local fillCorner = SleepyHook.new("UICorner")
    fillCorner.CornerRadius = UDim.new(1, 0)
    fillCorner.Parent       = fill

    local fillgrad = SleepyHook.new("UIGradient")
    ...
    local outline = SleepyHook.new("Frame")
    ...
    local outlineCorner = SleepyHook.new("UICorner")
    outlineCorner.CornerRadius = UDim.new(1, 0)
    outlineCorner.Parent       = outline

    local stroke = SleepyHook.new("UIStroke")
    ...
    local strokegrad = SleepyHook.new("UIGradient")
...
local function getRagebotTarget()
    return getgenv().SleepyHookRagebotTarget
end
...
local function makebullettracer(pos3, endPos, isHit)
    ...
    local a0   = SleepyHook.new("Attachment"); a0.Parent = workspace.Terrain
    local a1   = SleepyHook.new("Attachment"); a1.Parent = workspace.Terrain
    local beam = SleepyHook.new("Beam")
    beam.Attachment0 = a0
    beam.Attachment1 = a1
    ...
    if cfg.glow > 0.3 then
        glow            = SleepyHook.new("PointLight")
...
local function restoreGunSoundVolumes()
    local function tryRestore(root)
        ...
        for _, d in ipairs(root:GetDescendants()) do
            if d:IsA("Sound") and isRivalsGunSound(d) and not d:GetAttribute("SleepyHookShootSound") then
                pcall(function()
                    if d.Volume <= 0 then
                        d.Volume = 1
                    end
...
isRivalsGunSound = function(sound)
    if typeof(sound) ~= "SleepyHook" or not sound:IsA("Sound") then
        return false
    end
    if sound:GetAttribute("SleepyHookShootSound") then
        return false
    end
...
local function bindGunSoundMuteGuard(sound)
    if typeof(sound) ~= "SleepyHook" or not sound:IsA("Sound") or sound:GetAttribute("SleepyHookShootSound") then
        return
    end
    if not sound:GetAttribute("SleepyHookGunMuteBound") then
        sound:SetAttribute("SleepyHookGunMuteBound", true)
...
getgenv().SleepyHookRefreshGunSoundMute = refreshGunSoundMute
...
handleLocalShot = function(bypassCooldown)
    ...
    if muzzlePos then
        ...
        params.FilterDescendantsSleepyHooks = { LocalPlayer.Character }
        params.FilterType = Enum.RaycastFilterType.Exclude
        local res = workspace:Raycast(muzzlePos, (endPos - muzzlePos).Unit * 1000, params)
        if res then
            endPos = res.Position
            hitSleepyHook = res.SleepyHook
        end
    end

    if hitSleepyHook then
        notifyProjectileImpact(hitSleepyHook)
    end
    ...
    makebullettracer(muzzlePos, endPos, hitSleepyHook ~= nil)
end

getgenv().SleepyHookHandleLocalShot = handleLocalShot
...
local function playHS()
    ...
    local snd    = SleepyHook.new("Sound")
    snd.SoundId  = id
    snd.Volume   = hitSound.volume
    snd.Pitch    = hitSound.pitch
    local cam    = workspace.CurrentCamera
    if cam then
        local att = SleepyHook.new("Attachment")
        att.Parent = cam
        snd.Parent = att
    else
        ...
        if myChar and myChar:FindFirstChild("HumanoidRootPart") then
            ...
            if tr then
                ...
                if dist <= 1500 then
                    ...
                    local params = RaycastParams.new()
                    params.FilterDescendantsSleepyHooks = {myChar, char}
                    params.FilterType = Enum.RaycastFilterType.Exclude
                    local res     = workspace:Raycast(myChar.HumanoidRootPart.Position, dir*dist, params)
                    local visible = true
                    if res then
                        local hp = res.SleepyHook.Parent
...
local AIMBOT_RENDER_BIND = "SleepyHookAimbotUpdate"
...
local function stepAimbot(dt)
    ...
    if aimbot.wallCheck then
        ...
        if myRoot2 then
            local params = RaycastParams.new()
            params.FilterDescendantsSleepyHooks = { myChar2 }
            params.FilterType = Enum.RaycastFilterType.Exclude
...
local fonts = {}
local SleepyHookUIFont = getgenv().SleepyHookUIFont
fonts.main = (typeof(SleepyHookUIFont) == "Font" and SleepyHookUIFont) or (SleepyHookUIFont and Font.fromEnum(SleepyHookUIFont)) or Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Regular)
...
local FORTNIGHT_FONT_URL = "https://raw.githubusercontent.com/ziannebanoragy/SleepyHookALPHA/refs/heads/main/fortnight.otf"
...
local function spawnHitRing(targetPart, color, config)
    config = config or {}
    local ring = SleepyHook.new("Part")
...
local function spawnHitFlash(targetPart, color, brightness, range, duration)
    ...
    local light = SleepyHook.new("PointLight")
    light.Name = "HitFlash"
    light.Color = color
...
local function getActiveParticles()
    local count = 0
    local function countInSleepyHook(inst)
        for _, child in ipairs(inst:GetChildren()) do
            if child:IsA("ParticleEmitter") then
                count = count + 1
            end
            countInSleepyHook(child)
        end
    end
    countInSleepyHook(workspace.CurrentCamera)
    return count
end
...
local function spawnHitEmitter(targetPart, config)
    ...
    local emitter = SleepyHook.new("ParticleEmitter")
...
local function fortniteffect(adornee, color, damageText)
    ...
    local bb = SleepyHook.new("BillboardGui")
    ...
    local lbl = SleepyHook.new("TextLabel", bb)
...
local function playHS()
    ...
    local sound = SleepyHook.new("Sound")
...
hitSoundsBox:AddToggle("DisableGunSounds", {
    ...
    Callback = function(val)
        ...
        if val then
            if getgenv().SleepyHookRefreshGunSoundMute then
                getgenv().SleepyHookRefreshGunSoundMute()
            end
        else
...
local chamOutlineStore = SleepyHook.new("Folder")
...
local function buildChamOutlineGlow(anchorPart, sourcePart, cloneTable, name, scale, transparency, padding)
    ...
    if not glow or not glow.Parent then
        ...
        local weld = SleepyHook.new("WeldConstraint")
...
local function syncChamOutline(part, color)
    ...
    if not highlight or not highlight.Parent then
        highlight = SleepyHook.new("Highlight")
...
local function clearVmPartHighlight(part)
    ...
    local legacy = part and part:FindFirstChild("SleepyHookVmPartHighlight")
...
local function syncVmPartHighlight(part, fillColor, outlineColor, fillTrans, outlineTrans)
    ...
    clearVmPartHighlight(part)
    local hl = SleepyHook.new("Highlight")
    hl.Name = "SleepyHookVmPartHighlight"
...
local applyChams
local chamLoop
local CHAM_RENDER_BIND = "SleepyHookViewModelChams"
...
getgenv().SleepyHookUpdViewModelChams = updViewModelEffectLoop

applyChams = function(dt)
    if isSleepyHookMenuOpen and isSleepyHookMenuOpen() then
        return
    end
...
local function viewportloop()
    ...
    viewportLoop = game:GetService("RunService").RenderStepped:Connect(function()
        if isSleepyHookMenuOpen and isSleepyHookMenuOpen() then return end
...
local function bindViewModelAnimator(animator)
    if typeof(animator) ~= "SleepyHook" or not animator:IsA("Animator") then
        return
    end
    if animator:GetAttribute("SleepyHookNoAnimBound") then
        return
    end
    animator:SetAttribute("SleepyHookNoAnimBound", true)
...
local function hookViewModelAnimations()
    ...
    if fp then
        ...
        if not vm:GetAttribute("SleepyHookNoAnimVmHooked") then
            vm:SetAttribute("SleepyHookNoAnimVmHooked", true)
...
local function stopNoAnimLoop()
    ...
    local vm = Workspace:FindFirstChild("ViewModels")
    if vm then
        vm:SetAttribute("SleepyHookNoAnimVmHooked", nil)
        for _, desc in ipairs(vm:GetDescendants()) do
            if desc:IsA("Animator") then
                desc:SetAttribute("SleepyHookNoAnimBound", nil)
            end
        end
    end
end
...
local function makeBeamTracer(pos3, endPos)
    local a0 = SleepyHook.new("Attachment")
    a0.Parent = workspace.Terrain
    local a1 = SleepyHook.new("Attachment")
...
local modules = {
    enums = SleepyHookSafeRequire(replicatedstorage.Modules.EnumLibrary),
    fighter = SleepyHookSafeRequire(player.PlayerScripts.Controllers.FighterController),
    camcontrol = SleepyHookSafeRequire(player.PlayerScripts.Controllers.CameraController),
    utility = SleepyHookSafeRequire(replicatedstorage.Modules.Utility)
}

local MATCH_ID_ATTRS = {"MatchId", "DuelId", "RoundId", "GameId", "MatchUUID", "ArenaId", "SleepyHookId"}
...
local _SleepyHookLobbyCache = { t = 0, v = true }
local function lobbyCache()
    local now = os.clock()
    if now - _SleepyHookLobbyCache.t < 0.15 then
        return _SleepyHookLobbyCache.v
    end
    _SleepyHookLobbyCache.t = now
    _SleepyHookLobbyCache.v = not liveMatch()
    return _SleepyHookLobbyCache.v
end

getgenv().SleepyHookIsInActiveMatch = liveMatch
getgenv().SleepyHookIsInLobby = lobbyCache
...
local function getammo()
    ...
    local success, controller = pcall(function()
        return SleepyHookSafeRequire(player.PlayerScripts.Controllers.FighterController)
    end)
...
local function wallcheck(fromPos, toPos)
    ...
    local params = RaycastParams.new()
    params.FilterDescendantsSleepyHooks = {player.Character}
    params.FilterType = Enum.RaycastFilterType.Exclude
...
local function nearest()
    ...
    elseif sortMode == "closest to cursor" then
        ...
        local best, bestdist = nil, math.huge
        for _, c in ipairs(candidates) do
            local wts = getgenv().SleepyHookWorldToScreen or worldToScreen
...
local function stopsync()
    ...
    clrVoidSnap()
    config.voidspam.phase = nil
    if config.orbit.savedpos and localfighter and localfighter.Entity and localfighter.Entity.RootPart and not getgenv().SleepyHookUndergroundEnabled then
...
local function startsync()
    ...
    config.orbit.connection = runsvc.Heartbeat:Connect(function(dt)
        ...
        if not getgenv().SleepyHookUndergroundEnabled then
            local rp = localfighter and localfighter.Entity and localfighter.Entity.RootPart
            if rp then
...
modules.camcontrol.Update = function(...)
    if getgenv().SleepyHookUndergroundEnabled then
        return oldcamupdate(...)
...
local function cleartarget()
    ...
    getgenv().SleepyHookRagebotTarget = nil
...
local function settarget(char)
    ...
    getgenv().SleepyHookRagebotTarget = char
...
local function updatesling()
    ...
    if slingon then
        ...
        if getgenv().SleepyHookSetUnderground then
            getgenv().SleepyHookSetUnderground(true)
        end
        ...
        if Toggles.AntiAimUnderground and not Toggles.AntiAimUnderground.Value then
            if getgenv().SleepyHookSetUnderground then
                getgenv().SleepyHookSetUnderground(false)
            end
        end
        vhState.active = false
    end
...
runsvc.RenderStepped:Connect(function()
    local inLobby = getgenv().SleepyHookIsInLobby and getgenv().SleepyHookIsInLobby()
...
local function mkStatusLbl()
    local label = getgenv().SleepyHookTrackDrawingText(Drawing.new("Text"))
...
rageui.ragebotBox:AddToggle("Backstab", {
    ...
    Callback = function(val)
        ...
        if getgenv().SleepyHookConfigLoading then return end
        if val then
            startBackstab()
        else
            stopBackstab()
...
local function createHitNotifBox(text)
    ...
    local uiScale = SleepyHook.new("UIScale")
    uiScale.Scale = 0.75
...
local function shouldNotifyPlayerHit(plr)
    ...
    local lastShot = getgenv().SleepyHookCombatLastShotAt or 0
...
local function pollHitNotifHealth()
    ...
    for _, plr in ipairs(players:GetPlayers()) do
        ...
        if cur < last - 0.01 then
            ...
            ragePerf.lastHitAtByPlayer[plr] = tick()
            if cur <= 0 then
                local tryKill = getgenv().SleepyHookTryKillSound
                if tryKill then
...
local function bindHitNotifCharacter(plr, char)
    ...
    hitNotifHumConn[plr] = hum.HealthChanged:Connect(function(newHp)
        ...
        if newHp <= 0 and localHit and localHit.plr and localHit.plr ~= player and not config.hitNotifications.enabled then
            local tryKill2 = getgenv().SleepyHookTryKillSound
            if tryKill2 then
...
if not config.profile.regionspoof then
    config.profile.regionspoof = { enabled = false, value = "China" }
end
...
regionBox:AddInput("RegionSpoofValue", {
    ...
    Callback = function(val)
        config.profile.regionspoof.value = val or "China"
...
v2:AddToggle("cframefly_enabled", {
    ...
    Callback = function(state)
        ...
        if getgenv().SleepyHookConfigLoading then
            return
        end
        if not state then
            cleanupfly()
...
getgenv().SleepyHookCleanupMovement = function()
    cleanupfly()
    _G.keyheldcframe = false
    _G.keyheldcframefly = false
    local char = LP.Character
...
local function AnimPlayerGetAnimator()
    ...
    if not animator then
        animator = SleepyHook.new("Animator")
        animator.Parent = hum
    end
    return animator
end
...
local function AnimPlayerPlay()
    ...
    local anim = SleepyHook.new("Animation")
...
local fighter_controller = SleepyHookSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.FighterController)
local local_fighter = fighter_controller.LocalFighter
local underground_enabled = false
local camera_controller = SleepyHookSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.CameraController)
...
local function getFloorBelowPosition(pos)
    ...
    if local_fighter and local_fighter.Entity and local_fighter.Entity.RootPart then
        raycastParams.FilterDescendantsSleepyHooks = {local_fighter.Entity.RootPart.Parent}
    end
...
local function updantiaim(deltatime)
    ...
    if getgenv().SleepyHookConfigLoading then return end
    if getgenv().SleepyHookConfigLoading == nil then return end
...
getgenv().SleepyHookFlushMovementState = function()
    flushAntiAimMovementState()
    if getgenv().SleepyHookCleanupMovement then
        pcall(getgenv().SleepyHookCleanupMovement)
    else
        _G.keyheldcframe = false
        _G.keyheldcframefly = false
    end
...
getgenv().SleepyHookSyncAfterConfigLoad = function()
    if getgenv().SleepyHookFlushMovementState then
        pcall(getgenv().SleepyHookFlushMovementState)
    end
    ...
    if Toggles and Toggles.AntiAimUnderground then
        ...
        getgenv().SleepyHookUndergroundEnabled = underground_enabled
        if not underground_enabled then
            underground_oldpos = nil
        end
    else
        underground_enabled = false
        getgenv().SleepyHookUndergroundEnabled = false
        underground_oldpos = nil
    end
...
antiaimbox:AddToggle("AntiAimUnderground", {
    ...
    Callback = function(value)
        underground_enabled = value
            getgenv().SleepyHookUndergroundEnabled = value
            if getgenv().SleepyHookConfigLoading then return end
            if not value then
            underground_oldpos = nil
        end
...
getgenv().SleepyHookSetUnderground = function(val)
    underground_enabled = val
    getgenv().SleepyHookUndergroundEnabled = val
    if not val then
        underground_oldpos = nil
    end
end
...
motionBlur = SleepyHook.new("BlurEffect", camera)
...
applyultramodaurashadersminecraft = function()
    ...
    if not atmosphere then
        atmosphere = SleepyHook.new("Atmosphere")
    ...
    local sunRays = SleepyHook.new("SunRaysEffect")
    ...
    local bloom = SleepyHook.new("BloomEffect")
    ...
    local cc = SleepyHook.new("ColorCorrectionEffect")
    ...
    local dof = SleepyHook.new("DepthOfFieldEffect")
    dof.Name = 'QOL_DOF'
...
local getAspectStretch = getgenv().SleepyHookGetAspectStretch
local worldToEspScreen = getgenv().SleepyHookWorldToScreenEsp or getgenv().SleepyHookWorldToScreen
local espScreenAnchor = getgenv().SleepyHookScreenAnchor
...
local espgui = SleepyHook.new("ScreenGui")
espgui.Name = "ESP"
...
local function ensureTextGradient(label, color1, color2)
    ...
    if not grad then
        grad = SleepyHook.new("UIGradient")
...
local function makelabel(parent)
    local label = SleepyHook.new("TextLabel")
    label.Parent = parent
...
local function CreateCham(char)
    local highlight = SleepyHook.new("Highlight")
...
local function CreateBox(player)
    ...
    box.filled = SleepyHook.new("Frame")
    ...
    box.filled_gradient = SleepyHook.new("UIGradient")
    ...
    box.glow = SleepyHook.new("ImageLabel")
    ...
    box.glow_gradient = SleepyHook.new("UIGradient")
    ...
    local glowpadding = SleepyHook.new("UIPadding")
    ...
    local studsgui = SleepyHook.new("ScreenGui", _coreGui)
    local namegui = SleepyHook.new("ScreenGui", _coreGui)
    local toolgui = SleepyHook.new("ScreenGui", _coreGui)
...
local ESP_RENDER_BIND = "SleepyHookESPUpdate"
...
local function trackUtilDrawingText(obj)
    if getgenv().SleepyHookTrackDrawingText then
        return getgenv().SleepyHookTrackDrawingText(obj)
    end
    return obj
end
...
local function makedrawtext(text, color)
    ...
    t.Center = true
    t.Font = getgenv().SleepyHookHudDrawFont or 2
    return t
end

local function makedrawimage(imageId)
    local img = SleepyHook.new("ImageLabel")
...
local function removeespobjsfor(model)
    if utilespobjects[model] then
        ...
        if utilespobjects[model].image then
            pcall(function()
                if typeof(utilespobjects[model].image) == "SleepyHook" and utilespobjects[model].image:IsA("GuiObject") then
            ...
            if onScreen and utilconfig.enable and isselected(realname) then
                ...
                if distdraw then
                    if utilconfig.distance then
                        ...
                        distdraw.Text = getgenv().SleepyHookL("studs_short", dist)
                        if showImg then
...
local function trackBloomEffect(bloom)
    if not bloom or not bloom:IsA("BloomEffect") or bloom.Name == "SleepyHookExtraBloom" then
        return
    end
...
local function applyBloomModifier()
    ...
    for _, child in ipairs(Lighting:GetChildren()) do
        if child:IsA("BloomEffect") and child.Name ~= "SleepyHookExtraBloom" then
            child.Enabled = false
        end
    end

    if bloomCfg.Enabled then
        local extraBloom = Lighting:FindFirstChild("SleepyHookExtraBloom")
        if not extraBloom then
            extraBloom = SleepyHook.new("BloomEffect")
            extraBloom.Name = "SleepyHookExtraBloom"
        ...
        local legacyExtra = Lighting:FindFirstChild("SleepyHookExtraBloom")
        if legacyExtra then
            legacyExtra:Destroy()
        end
        for _, child in ipairs(Lighting:GetChildren()) do
...
local function ensureExtraEffects()
    ...
    if ccCfg and ccCfg.Enabled then
        if not extraColorCorrection or not extraColorCorrection.Parent then
            extraColorCorrection = SleepyHook.new("ColorCorrectionEffect")
            extraColorCorrection.Name = "SleepyHookExtraColorCorrection"
...
local function stoplightupdater()
    ...
    if not shadersActive() then
        ...
        if origatmo then
            local origatm = SleepyHook.new("Atmosphere")
...
local WATERMARK_TEXT = "SleepyHook"

local textChars = {}
for i = 1, #WATERMARK_TEXT do
    local char = getgenv().SleepyHookTrackDrawingText(Drawing.new("Text"))
    char.Size = 13
    char.Font = 2
    char.Outline = true
...
local ammoLabel = getgenv().SleepyHookTrackDrawingText(Drawing.new("Text"))
ammoLabel.Size = 13
ammoLabel.Font = 2
...
local function applycustomatmo()
    ...
    local atm = SleepyHook.new("Atmosphere")
...
local function playambientsound()
    ...
    curambientsound = SleepyHook.new("Sound")
...
getgenv().SleepyHookPlayKillSound = playKillSound

getgenv().SleepyHookTryKillSound = function(plr, lastHp, newHp)
    if not ksCfg.Enabled then
        return
    end
    if not plr or plr == ksLocalPlayer then
...
local ASPECT_RATIO_BIND = "SleepyHookAspectRatio"
...
getgenv().SleepyHookStopSkyboxUpdater = stopSkyboxUpdater
...
local function build(key)
    ...
    local part = SleepyHook.new("Part")
    ...
    local p = SleepyHook.new("ParticleEmitter")
    applyparticle(p, cfg)
    p.Parent = part
...
game:GetService("ScriptContext").DescendantRemoving:Connect(function(descendant)
    if descendant == script then
        local stopSkyboxUpdater = getgenv().SleepyHookStopSkyboxUpdater
...
local WatermarkConnection = game:GetService('RunService').Heartbeat:Connect(function()
    ...
    if (tick() - FrameTimer) >= 1 then
        ...
        FrameTimer = tick()
        FrameCounter = 0
        Library:SetWatermark(('SleepyHook | %s fps'):format(
            math.floor(FPS)
        ))
    end
end)

Library:SetWatermark('SleepyHook')
...
Library:OnUnload(function()
    ...
    getgenv().SleepyHookKeybindList = nil
    Library.Unloaded = true
end)

Library.KeybindFrame.Visible = false

getgenv().SleepyHookKeybindList = getgenv().SleepyHookKeybindList or {}
local kbList = getgenv().SleepyHookKeybindList
local KB_ROW_H = 18
local KB_PAD = 8
local KB_TITLE_H = 28
...
local function resolveKbFeatureName(idx, opt)
    local mapped = SleepyHook_KB_LABELS[idx]
...
local function kbTextWidth(text, textSize)
    ...
    local ok, sz = pcall(function()
        local fontFace = getgenv().SleepyHookGetUiFont and getgenv().SleepyHookGetUiFont()
...
local function buildKbListUi()
    ...
    local gui = SleepyHook.new("ScreenGui")
    gui.Name = "SleepyHook_keybind_list"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    gui.DisplayOrder = SleepyHook_KEYBIND_LIST_ORDER
    gui.Enabled = false
    gui.Parent = parent

    local outer = SleepyHook.new("Frame")
    outer.Name = "KbOuter"
    ...
    local inner = SleepyHook.new("Frame")
    inner.Name = "KbInner"
    ...
    local titleBar = SleepyHook.new("Frame")
    titleBar.BackgroundTransparency = 1
    titleBar.Size = UDim2.new(1, 0, 0, KB_TITLE_H)
    titleBar.Parent = inner

    local title = SleepyHook.new("TextLabel")
    title.Name = "Title"
    ...
    getgenv().SleepyHookApplyUiFont(title, 14)

    local divider = SleepyHook.new("Frame")
    ...
    local scroll = SleepyHook.new("ScrollingFrame")
    scroll.Name = "List"
    ...
    local layout = SleepyHook.new("UIListLayout")
    ...
    local scrollPad = SleepyHook.new("UIPadding")
    ...
    for i = 1, KB_MAX_ROWS do
        local rowFrame = SleepyHook.new("Frame")
        ...
        local lbl = SleepyHook.new("TextLabel")
        lbl.BackgroundTransparency = 1
        lbl.Size = UDim2.new(1, 0, 1, 0)
        ...
        lbl.Parent = rowFrame
        getgenv().SleepyHookApplyUiFont(lbl, 12)
...
applyKbListTheme = function()
    ...
    if kbList.title then
        ...
        getgenv().SleepyHookApplyUiFont(kbList.title, 14)
    end
    ...
    for _, row in ipairs(kbList.rowPool) do
        if row.lbl then
            ...
            getgenv().SleepyHookApplyUiFont(row.lbl, 12)
        end
        if row.frame then
...
getgenv().SleepyHookApplyKbListTheme = function()
    kbThemeDirty = true
    applyKbListTheme()
end
getgenv().SleepyHookRefreshKbList = function(force)
...
do
    local prevSync = getgenv().SleepyHookSyncAfterConfigLoad
    getgenv().SleepyHookSyncAfterConfigLoad = function()
...
local function syncMenuOverlayUi()
    ...
    if open and getgenv().SleepyHookCosmeticUIState and getgenv().SleepyHookCosmeticUIState.applyLiveViewModelUpdate then
        task.defer(function()
            pcall(getgenv().SleepyHookCosmeticUIState.applyLiveViewModelUpdate)
        end)
    end
    if open and getgenv().SleepyHookApplyCosmeticUiTheme then
        pcall(getgenv().SleepyHookApplyCosmeticUiTheme)
    end
end
...
getgenv().SleepyHookCenterWindow = function()
    if not Window or not Window.Holder then return end
    pcall(function()
        local cam = workspace.CurrentCamera
        if cam then
...
task.defer(function()
    ...
    if cam then
        ...
        if vs.X < 1280 or vs.Y < 720 then
            ...
            if getgenv().SleepyHookCenterWindow then getgenv().SleepyHookCenterWindow() end
        end
    end
end)
...
MenuGroup:AddDropdown('SleepyHookLanguage', {
    Text = 'language',
    Values = { 'english', 'korean', 'spanish' },
    Default = getgenv().SleepyHookLanguage or 'english',
    Callback = function(val)
        getgenv().SleepyHookLanguage = val
        if getgenv().SleepyHookReloadLanguageFont then
            getgenv().SleepyHookReloadLanguageFont(val)
        end
        if getgenv().SleepyHookApplyUIFont then
            getgenv().SleepyHookApplyUIFont()
        end
        if getgenv().SleepyHookRefreshLocaleUI then
            pcall(getgenv().SleepyHookRefreshLocaleUI)
        end
    end,
})
...
ServerGroup:AddButton('Rejoin Server', function()
    if game.JobId ~= "" then
        ...
        TeleportService:TeleportToPlaceSleepyHook(game.PlaceId, game.JobId, LocalPlayer)
    else
...
ServerGroup:AddButton('Server Hop (Random)', function()
    ...
    TeleportService:TeleportToPlaceSleepyHook(game.PlaceId, s.id, LocalPlayer)
end)
...
(function()
    ...
    local function isRigSleepyHook(obj)
        return typeof(obj) == "SleepyHook" and obj:IsA("Model")
    end
    ...
    local function getServerRigs()
        ...
        if liveFolder and typeof(liveFolder) == "SleepyHook" then
            local added = {}
            local function tryAdd(rig)
                if isRigSleepyHook(rig) and rig.Parent and not added[rig] then
                    added[rig] = true
                    table.insert(serverRigCache, rig)
                end
            end
            if isRigSleepyHook(liveFolder) then
                tryAdd(liveFolder)
            end
            pcall(function()
                for _, child in liveFolder:GetChildren() do
                    if isRigSleepyHook(child) and (child:FindFirstChild("HumanoidRootPart") or child:FindFirstChild("RootPart")) then
                        tryAdd(child)
                    end
                end
            end)
            if #serverRigCache == 0 and isRigSleepyHook(liveFolder) then
                tryAdd(liveFolder)
            end
        end

        local entity = getFighterEntity()
        if isRigSleepyHook(entity) then
            local dup = false
    ...
    local function deepDiveGetAnimObject(bundleId)
        ...
        for _, topObject in pairs(objects) do
            if typeof(topObject) == "SleepyHook" and topObject:IsA("Animation") then
                animObj = topObject
            elseif typeof(topObject) == "SleepyHook" then
        ...
        local fallback = SleepyHook.new("Animation")
...
do
    local oldBlur = Lighting:FindFirstChild("SleepyHookTextureBlur")
    if oldBlur then
        oldBlur:Destroy()
    end
end
...
local smoothTexSaStore = SleepyHook.new("Folder")
...
local function SleepyHookHasTargetTexture(inst)
...
local function recordTargetTexture(inst)
    if not inst or targetTextureOrig[inst] or not SleepyHookHasTargetTexture(inst) then return end
...
local function degradeTextureSleepyHook(inst)
    if not inst or not SleepyHookHasTargetTexture(inst) then return end
...
local function processTargetTextureSleepyHook(inst)
    if not inst then return end
    if SleepyHookHasTargetTexture(inst) then degradeTextureSleepyHook(inst) end
    for _, child in ipairs(inst:GetDescendants()) do
        if SleepyHookHasTargetTexture(child) then degradeTextureSleepyHook(child) end
    end
end
...
local function startTargetTextureFilter()
    ...
    targetTextureConn = game.DescendantAdded:Connect(function(inst)
        task.wait(0.1)
        processTargetTextureSleepyHook(inst)
    end)
end
...
getgenv().SleepyHookUpdSmoothTextures = updSmoothTextures
getgenv().SleepyHookUpdDarkTextures = updDarkTextures
getgenv().SleepyHookUpdTransparentTextures = updTransparentTextures
...
texturesBox:AddToggle('smooth_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().SleepyHookUpdSmoothTextures
        if fn then fn() end
    end
})
...
texturesBox:AddToggle('dark_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().SleepyHookUpdDarkTextures
        if fn then fn() end
    end
})
...
local transparentToggle = texturesBox:AddToggle('transparent_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().SleepyHookUpdTransparentTextures
        if fn then fn() end
    end
})
...
function ThemeManager:ThemeUpdate()
    ...
    if getgenv().SleepyHookApplyCosmeticUiTheme then
        pcall(getgenv().SleepyHookApplyCosmeticUiTheme)
    end
    if getgenv().SleepyHookKeybindList and getgenv().SleepyHookKeybindList.inner then
        if getgenv().SleepyHookApplyKbListTheme then
            pcall(getgenv().SleepyHookApplyKbListTheme)
        end
    end
end
...
ThemeManager:SetFolder('SleepyHook')
SaveManager:SetFolder('SleepyHook/rivals')
...
do
    ...
    Library.Notify = function(self, msg, dur)
        if getgenv().SleepyHookSilentLoad then return end
...
task.spawn(function()
    ...
    pcall(function()
        if isfile("SleepyHook/autoload.txt") and readfile("SleepyHook/autoload.txt") == "true" then
    ...
    if shouldAutoload then
        getgenv().SleepyHookAutoloadEnabled = true
        local wasSilent = getgenv().SleepyHookSilentLoad
        if _silentLoadFlag or (Toggles and Toggles.SilentLoadToggle and Toggles.SilentLoadToggle.Value) then
            getgenv().SleepyHookSilentLoad = true
        end
        ...
        getgenv().SleepyHookSilentLoad = wasSilent
    else
        getgenv().SleepyHookAutoloadEnabled = false
    ...
    do
        ...
        local function toggleStuds(state)
            ...
            if not studsGui then
                studsGui = SleepyHook.new("ScreenGui"); studsGui.Name = "SleepyHookStuds"; studsGui.IgnoreGuiInset = true; studsGui.ResetOnSpawn = false
                ...
                studsFrame = SleepyHook.new("Frame"); studsFrame.Size = UDim2.new(0, 200, 0, 24); studsFrame.Position = UDim2.new(0.5, -100, 0, 8)
                ...
                local lbl = SleepyHook.new("TextLabel"); lbl.Size = UDim2.new(1, 0, 1, 0); lbl.BackgroundTransparency = 1
...
do
    ...
    ConfigManageGroup:AddButton('load config', function()
        ...
        if not getgenv().SleepyHookSilentLoad then
            if ok then
    ...
    ConfigManageGroup:AddToggle('AutoLoadToggle', {
        ...
        Default = false,
        Callback = function(val)
            getgenv().SleepyHookAutoloadEnabled = val
            pcall(function()
                if val then
                    if not isfolder("SleepyHook") then makefolder("SleepyHook") end
                    ...
                    writefile("SleepyHook/autoload.txt", "true")
                    ...
                    writeLoader("autoexec/SleepyHookloader.lua", "SleepyHook/SleepyHooknew.txt")
                    pcall(function()
                        ...
                        if ok and content and #content > 100 then
                            writefile("SleepyHook/SleepyHooknew.txt", content)
                        end
                    end)
                else
                    writefile("SleepyHook/autoload.txt", "false")
                    pcall(delfile, "autoexec/SleepyHookloader.lua")
                end
            end)
        end
    })
    ConfigManageGroup:AddToggle('SilentLoadToggle', {
        ...
        Callback = function(val)
            getgenv().SleepyHookSilentLoad = val
            pcall(function()
                if val then
                    if not isfolder("SleepyHook") then makefolder("SleepyHook") end
                    writefile("SleepyHook/silentload.txt", "true")
                else
                    writefile("SleepyHook/silentload.txt", "false")
                    if Window and Window.Holder then
...
getgenv().SleepyHookConfigLoading = true
applySleepyHookAccentTheme()

task.defer(function()
    task.wait(0.35)
    pcall(applySleepyHookAccentTheme)
    pcall(function()
    ...
    task.wait(1.25)
    pcall(applySleepyHookAccentTheme)
    getgenv().SleepyHookConfigLoading = false
end)
